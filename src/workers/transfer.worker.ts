...
const workerTransfer = new Worker(
    ...
    async (job: Job) => {
        ...
        const { amount, failure_reason, idempotency_key, payee_id, payer_id, status } = job.data
...
workerTransfer.on('error', (error: Error) => {
    console.error(`WORKER [transfer-queue] Error`, error)
})

workerTransfer.on('failed', (job, error) => {
    console.error(`WORKER [transfer-queue] Job ${job?.id} failed`, error)
})
